...
use landscape::build::{build, BuildArgs};
use landscape::deploy::s3::{self};
use landscape::deploy::{DeployArgs, Provider};
use landscape::new::{new, NewArgs};
use landscape::serve::{serve, ServeArgs};
use landscape::validate::{
    ...
    match &cli.command {
        Command::Build(_) | Command::Deploy(_) | Command::New(_) | Command::Serve(_) => {
            if std::env::var_os("RUST_LOG").is_none() {
                std::env::set_var("RUST_LOG", "landscape=debug");
            }
